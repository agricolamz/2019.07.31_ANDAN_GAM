...
```{r}
poly_2_fit3 <- lm(y ~ bs(x), data = poly_2)
summary(poly_2_fit3)

poly_2_fit3_values <- tibble(x = poly_2$x,
...
```{r}
poly_2_fit4 <- gam(y ~ s(x), data = poly_2)
summary(poly_2_fit4)

fitted <- tibble(x = poly_2$x,
                 fit = poly_2_fit4$fitted.values)

poly_2 %>% 
  ggplot(aes(x, y))+
  geom_point()+
  geom_line(data = fitted, aes(x = x, y = fit), color = "blue")+
  labs(caption = poly_2_fit4$call)
```
