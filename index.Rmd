...
```{r, include=FALSE}
df <- read_csv("https://raw.githubusercontent.com/agricolamz/2019.08.01_ANDAN_GAM/master/data/avocado.csv")

df %>% 
  ggplot(aes(Date, AveragePrice, color = type))+
  geom_point()+
  facet_wrap(~region)+
  geom_smooth(method = "lm", formula = y ~ poly(x, 12), color = "black", aes(group = type))
# Pittsburgh
```
